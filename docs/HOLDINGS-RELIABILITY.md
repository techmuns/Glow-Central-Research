...
1. Connect an authorized primary exchange or licensed disclosure feed covering the full listed-company universe, including new companies not already held. The tested direct BSE shareholding API returned HTTP 403 in the read-only test; a manually readable page is not a reliable automated service contract. Primary holdings evidence is individually checked; the live bulk/block feed confirms reported trades but is not a full holdings reconciliation feed.
