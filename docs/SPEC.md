...
| 11 | AI Alerts cards built for time to insight | The ranking was already honest and the card still took twenty seconds to read: the leading pattern's sentence was printed as the insight AND again, verbatim, in a *Signals lining up* panel below it, in the feeds' own wording, above evidence rows carrying full timestamps, direction pills, importance pills and each rule's reason. It is now one short sentence in ordinary English and four dated evidence rows, newest first — with an **Archive** that is a place rather than a deletion. The four-figure strip and the per-question paragraph that sat between the sentence and those rows were removed on 19 September 2026 at the desk's request; every figure the strip carried has a place on the card still (the sentence's own subject, the direction dot on each row, the list header's source count, the footer's door to All Alerts), and the per-question reading moved onto the row whose record backs it, as a chip naming what that record could change. Every phrase rewords an event already on the card and every figure reads a field the collector writes (`volumeX`, `movePct`, `deltaPp`), never a regex over prose. Volume takes no colour, because participation has no sign. Rows go one per source in rounds, at most three from any one source and at most four in all: taking the top rows by score alone put three near-identical fund rows on a card announcing four sources, and counting sources by feed id rather than by family let one board meeting filed to both exchanges under four subjects take every row of a card whose header read one source. ✅ |
