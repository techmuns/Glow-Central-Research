...
| BENGALASM | The NSE quote was old. Use verified BSE code `533095`, ISIN `INE083K01017`, consistently for quote requests and future history. Never copy NSE volume/base history into a BSE quote. |
| VERTIS, NHIT, 543225 (Altius), 504375 (IDream) | These instrument identities exist. Keep the provider's feed-update timestamp separate from its last-trade timestamp. A fresh zero-volume session can have an older last trade; it must not manufacture today's breakout. A stale/missing provider timestamp or conflicting session volume remains a gap. Actual availability is verified after the automatic deployment, not inferred from successful mapping. |
