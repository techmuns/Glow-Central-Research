...
| Workspace | Tabs |
| --- | --- |
| Research Central | **Ask Research** (the landing tab, first in the bar) · AI Alerts · General Alerts · Earnings Hub · Con-call · Public Chatter · Breakouts / Technical · Super Investors · News · Corp Announcements · Insider Trades · Macro Research · Economy & Macro · Family Book (all three Glow-owned, last in the bar) |
...
```jsonc
// wrangler.jsonc
{ "name": "glow-central-research", "compatibility_date": "2026-05-23",
...
### What the Glow Worker and the Glow repository need configured

Every feature merged from Sattva that needs a credential names its own missing state on screen, so
nothing below breaks the page when absent — it just stays honest about what it cannot read.

**Worker secrets** — Cloudflare dashboard → *Workers & Pages → glow-central-research → Settings →
Variables and Secrets* (or `npx wrangler secret put <NAME>` from a terminal logged in to the account):
...
| `GLOWVENTURES_READ_TOKEN` | `series-refresh.yml` (**GlowVentures sync**) — copies the macro series store from the private techmuns/GlowVentures repository every morning **and rebuilds the family office book (`public/data/book.json`) and the family's managers (`public/data/managers.json`, My Managers on Superstar Investors) from the same checkout**, refusing to commit either file when its sums do not reconcile to the book's headline. A fine-grained PAT for that one repository with *Contents: read* | neither is refreshed; the macro tabs keep the committed store and print its harvest time, the Family Book tab, Ask Research and My Managers keep the committed book and managers and print their statement date |
...
than a loud one: both `wrangler deploy` and `wrangler secret put` would address a *second*, empty
Worker of that name, succeed, and leave the real site untouched. A secret in particular looks
perfectly set that way — it is just set somewhere nothing reads it.
