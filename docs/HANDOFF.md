...
| `GLOWVENTURES_READ_TOKEN` | `series-refresh.yml` (**GlowVentures sync**) — copies the macro series store from the private techmuns/GlowVentures repository every morning **and rebuilds the family office book (`public/data/book.json`) from the same checkout**, refusing to commit a book whose sums do not reconcile. A fine-grained PAT for that one repository with *Contents: read* | neither is refreshed; the macro tabs keep the committed store and print its harvest time, the Family Book tab and Ask Research keep the committed book and print its statement date |
