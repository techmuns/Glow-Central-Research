...
| Feed | Where | Notes |
| --- | --- | --- |
| **Additional corporate announcements** | Corp Announcements continuous feed | Scheduled Muns BSE/NSE/DRHP captures join BSE and live NSE announcements with source labels and document links. History loads automatically; the selected global scope filters the stream. Coverage gaps remain in the source information panel. See `docs/DATA-CONTRACTS.md` → Additional corporate-announcement lookups. |
