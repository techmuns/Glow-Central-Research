...
async function dailyRevision(origin,fetcher,edgeCache) {
  ...
  const commits=await boundedJson(await fetcher('https://api.github.com/repos/techmuns/Glow-Central-Research/commits?path=public%2Fdata%2Ftechnicals.json&per_page=1&sha=main',{
...
export async function handleTechnicals(request, env, { fetcher = fetch, edgeCache = globalThis.caches?.default } = {}) {
  ...
  try {
    ...
    const source = await fetcher(`https://raw.githubusercontent.com/techmuns/Glow-Central-Research/${revision}/public/data/${file}`, {
