...
export class BreakoutSchedule {
  ...
  async wake() {
    ...
    if (marketWindow(at).collect) {
      try {
        if (!this.env.GH_DISPATCH_TOKEN || this.env.GH_REPO !== 'techmuns/Glow-Central-Research' || (this.env.GH_REF || 'main') !== 'main') throw Error('configuration');
        const cfg = { token: this.env.GH_DISPATCH_TOKEN, owner: 'techmuns', repo: 'Glow-Central-Research', ref: 'main' };
