// Shared resolver for live holdings and scheduled snapshots. No filesystem or credentials.
const DISPLAY_NAMES = {
...
export function buildCandidates({ scans = {}, mc = {}, universe = {} } = {}) {
  const out = new Map();
  const add = (name, ticker, source) => {
    const t = String(ticker || '').trim().toUpperCase(), n = norm(name);
    if (t && n && !out.has(n)) out.set(n, { ticker: t, name: String(name).trim(), source, squashed: squash(n) });
  };
  for (const r of scans.rows || []) add(r.name, r.ticker, 'stockscans');
  for (const e of Object.values(mc.map || {})) add(e.fullName, e.ticker, 'moneycontrol');
  for (const row of Object.values(universe)) {
    const m = /company\/([A-Z0-9&.\-]+)\//i.exec(row?.['Screener URL'] || '');
...
export async function resolvePortfolio(fixture, sources, lookup = null) {
  const BOOK = fixture.lines;
  ...
  if (lookup) for (const u of unresolved.slice()) {
    const hit = await lookup(u.name);
    if (!hit) continue;
    holdings[u.index] = { ...holdings[u.index], ticker: hit.ticker, listed: true, matchedName: hit.name, matchedBy: 'yahoo-search' };
  ...
  const payload = {
    ...
    sourceWorkbook: fixture.sourceWorkbook || null,
    sourceRevision: fixture.revision || fixture.sourceRevision || null,
    storage: fixture.storage || null,
    syncedAt: fixture.checkedAt || fixture.fetchedAt || null,
