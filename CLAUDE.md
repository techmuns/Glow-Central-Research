...
2. **Every figure carries its own state and time.** Indian indices prefer the official NSE snapshot and BSE Indices' dated Sensex chart,
   with the existing server-side Upstox token and exact cash-index identities.
   A separate Upstox batch covers S&P 500, Dow, Nikkei and Hang Seng; other index families,
...
So `SAST_HOLDING_DISCLOSURE` drops Acquisition — only on the DISCLOSURE half of the regulations
(`SAST_DISCLOSURE_MARKER`: Regulations 29/31/10, a disclosure, shareholding, pledge or inter-se
transfer), never where `ACQUISITION_COMPLETED` says an acquisition completed, and never on an open
offer, its public announcement or letter of offer, which IS a takeover — and `LEGAL_ORDER` drops Order
and Receipt of Order; the reason string names what was dropped and why. A tax demand or penalty is still read, as a negative, by the
...
### THE TWO BULLETS AND THE TRIGGER CHIPS ARE RETIRED — the card is Sattva's (23 September 2026)

From 17 to 23 September every Glow card carried two bullets — *What happened* and *Earnings
assumption, valuation or thesis?* — the four-figure strip, and a **Could change · Earnings ·
Valuation · Thesis** chip group above the cards (`impactOf()` / `impactLine()`). Sattva's desk
removed the same paragraph and strip on 19 September (*"that the customer knows already"*), and on
23 September the owner asked for the two dashboards' cards to look the same. So Glow's card is now
the template's: one sentence, one list of dated rows, and the reading on the row that holds its
record (`driverReadings()` / `driverChipsMarkup()`, from `alert-drivers.js`), plus Glow's own
*KPIs in play* section below. `impactOf`, `impactLine`, `IMPACT_AXES` and the trigger chips are
deleted with the bullet they explained: a filter whose reason is no longer printed on the card
would narrow to cards that cannot say why they are there. **A later sync must not bring either
block back**, and the owner's desk brief behind them now reads off the row chips and the KPI line.

**Superseded the same evening by the customer's own request (23 September 2026):** every alert reads
as TWO bullets again — **What happened** (the lead development's LINE 1, e.g. *"Secures ₹2,600 Cr
redevelopment project in Goregaon"*) and **So what? · AI reading** (the likely earnings or valuation
implication). The retired blocks stay retired: the topic paragraph, the figure strip and the trigger
...
Research keeps the missing-consensus limitation with Earnings Hub: reported growth must never be
presented as an earnings surprise.

**The technical filter chips are shared across all three views**, and they now live in
`js/tabs/technical-filters.js`. Glow built them; the template extracted them into that module, so
Technical Scanner and FII Accumulation cannot drift from Strong Breakouts' Volume Confirm, 52W
Proximity and Trend Filter definitions. Each defaults to All. Counts hold the other chip groups
fixed within the current scope; FII's holding filters are included. Keep search and score
selections across chip repaints, and keep scanner cards, exports and price-refresh candidates
within the selected chips — the cards and the export, that is: the per-tab live-quote button that
used to POST the filtered tickers to `/api/live-prices` is gone, replaced by the app-wide Refresh
registry reading one collected `/api/breakouts` capture, so there are no per-row quote candidates
left to narrow. An id no group offers falls back to that group's default rather than reaching the
predicates, where it would empty the table while the chip bar highlighted nothing.
`scripts/verify-technical-filters.mjs` covers boundary values, missing
data and empty intersections offline. Two browser checks drive them in the real app and both run:
`verify-technical-filters-ui.mjs` is the template's, serves itself and is what CI invokes directly;
