...
test('GitHub OIDC checks the signature, audience, exact workflow, immutable repository IDs, ref and expiry', async () => {
  ...
  const base={iss:'https://token.actions.githubusercontent.com',aud:`${SUMMARY_ORIGIN}/api/concall-summaries/collector`,repository:'techmuns/Glow-Central-Research',repository_id:'1339395437',repository_owner_id:'278697674',
    ref:'refs/heads/main',workflow_ref:`techmuns/Glow-Central-Research/.github/workflows/${SUMMARY_WORKFLOW}@refs/heads/main`,event_name:'schedule',exp:START/1000+300,iat:START/1000,nbf:START/1000,run_id:'12',run_attempt:'1'};
...
test('durable timer is read-only until armed and keeps recovery after dispatch failure', async () => {
  const backing=storage();const env={SCREENER_SUMMARIES_ENABLED:'true',GH_REPO:'techmuns/Glow-Central-Research',GH_REF:'main'};
...
function timerHarness() {
  ...
  const env = { SCREENER_SUMMARIES_ENABLED: 'true', GH_REPO: 'techmuns/Glow-Central-Research', GH_REF: 'main', GH_DISPATCH_TOKEN: 'fixture' };
