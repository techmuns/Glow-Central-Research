...
function artifactFetch({ digest = null, host = 'https://example.blob.core.windows.net/capture', event = 'schedule' } = {}) {
  ...
  const run = { id: 10, head_branch: 'main', head_repository: { full_name: 'techmuns/Glow-Central-Research' }, event, status: 'completed', conclusion: 'success' };
...
function checkpointFetch({ outcome = 'calendar-shape', missingLatest = false, corrupt = false, expired = false,
  ...
  const run = id => ({ id, head_branch:'main', head_repository:{full_name:'techmuns/Glow-Central-Research'}, event:'schedule',
