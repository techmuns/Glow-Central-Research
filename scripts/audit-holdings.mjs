...
const report = assessCoverage({ exchange, snapshot: load('super-investors'), managers: load('managers'), deals: load('insider-trades'), evidence: load('holding-evidence') });
...
const operationalFailure = report.rows.some((r) => r.issues.some((issue) => /Book unavailable|Refresh failed|Source check overdue|archive sync not recently/.test(issue))) ||
  report.issues.some((issue) => /list unavailable|bulk\/block feed overdue|live delivery unavailable/i.test(issue));
