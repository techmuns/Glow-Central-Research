...
try {
  ...
  servedIndex.feeds.technicals.row.status = 'partial';
  const repeated = await read({ ...options, refresh: true });
  assert.equal(calls.length, 0, 'unchanged selective members survive an artifact change');
  assert.equal(repeated.feeds.get('technicals').status, 'partial', 'source health is rebuilt during selective reuse');
  assert.deepEqual(repeated.feeds.get('technicals').events, selective.feeds.get('technicals').events);
  const changedMember = servedIndex.days.find(entry => entry.day === day).feedMembers.technicals;
  changedMember.hash = 'b'.repeat(64); artifact++;
