// The shared filter chips exercised INSIDE another suite's server, against that suite's own book —
// `verify-glow-parity-ui.mjs` is the caller. The template's `verify-technical-filters-ui.mjs` is a
// standalone version that serves itself and is what CI invokes directly; this one covers the
// boundary values, the missing-data rows and the empty intersections, and needs a running origin.
...
export async function verifyTechnicalFiltersUI(browser, { base = 'http://127.0.0.1:8080' } = {}) {
  ...
  try {
    await context.route('**/*', route => {
      ...
      ` });
      // Kept so a reintroduced per-row quote request shows up rather than silently 404ing.
    ...
    // THE PRICE REFRESH NO LONGER CARRIES THE FILTERED TICKERS, BECAUSE IT NO LONGER CARRIES ANY.
    // This used to press a per-tab button that POSTed the selected rows to `/api/live-prices`, and
    // asserted the request named exactly the chip selection. The tab now registers with the app-wide
    // Refresh registry and reads one collected `/api/breakouts` capture, so there is no per-row
    // request left to narrow — the claim the assertion made is not false, it has no subject. What
    // the chips still own is what is DRAWN, which the export assertion above measures directly.
    assert.equal(await page.locator('[data-refresh-btn]').count(), 0, 'the per-tab quote button is gone, not merely hidden');
    ...
    // Same removal as above: with no per-tab quote button there is nothing here to disable. What
    // an empty intersection must still do is paint an empty table rather than fall back to the
    // whole set, which is the reading `click()` already asserted by passing `[]`.
    assert.equal(await page.locator('[data-refresh-btn]').count(), 0);
    ...
    assert.deepEqual(refreshes, [], 'nothing on this tab asks for quotes per filtered row any more');
