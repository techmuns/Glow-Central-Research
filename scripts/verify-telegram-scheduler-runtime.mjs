...
export class TestScheduler extends CaptureRegistry {
  constructor(ctx, env) {
    ...
    this.schedule.fetcher = async (url, init) => {
      if (!String(url).startsWith('https://api.github.com/repos/techmuns/Glow-Central-Research/')) throw Error('Unexpected destination');
      if (init.method === 'POST') {
...
writeFileSync(config, JSON.stringify({ name:'telegram-timer-local-test',main:fixture,compatibility_date:'2026-05-23',
  ...
  vars:{GH_REPO:'techmuns/Glow-Central-Research',GH_REF:'main',GH_DISPATCH_TOKEN:'fake-local-token'} }));
let child, logs = '';
