...
globalThis.location = { origin: 'https://glow-central-research.tech-441.workers.dev' };
const bridge = await import('../public/js/research/portfolio-bridge.js?transport');
const emit = (message, origin = 'https://glow-central-research.tech-441.workers.dev', source = parent) => { for (const fn of [...listeners]) fn({ origin, source, data: message }); };
responder = m => {
...
assert.equal(elements.find(e => e.tag === 'iframe').src, 'https://glow-central-research.tech-441.workers.dev/glow-bridge.html');
...
assert.ok(posts.every(p => p.origin === 'https://glow-central-research.tech-441.workers.dev'));
...
assert.match(direct.reading.answer, /source statement marks/);
