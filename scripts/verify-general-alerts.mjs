...
// The window is written off the collection's path, so wait on the write rather than the clock:
// this pool fills many cache parts and a bare read here would race a perfectly healthy save.
await alerts.whenAlertWindowSaved();
