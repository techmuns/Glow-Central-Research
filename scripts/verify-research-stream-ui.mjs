...
try {
  ...
  await context.route('**/*', route => {
    ...
    if (url.origin === familyOrigin && url.pathname === '/glow-bridge.html') return route.fulfill({ contentType: 'text/html', body: familyHtml });
  ...
  const family = page.frames().find(frame => frame.url() === `${familyOrigin}/glow-bridge.html`);
  assert(family);
  ...
  const safeRender = await page.evaluate(async () => {
    ...
    renderResearchAnswer(node, 'Portfolio reading [Dashboard: Ask Glow]', { compactCitations: true, cite: () => ({ href: 'https://glowventures.example/ask', label: 'Ask Glow' }) });
