...
export async function loadActivePortfolio(path, { live = process.env.FAMILY_HOLDINGS_LIVE === 'true', fetcher = fetch, previous = null } = {}) {
  if (!live) return JSON.parse(readFileSync(path, 'utf8'));
  const response = await fetcher('https://glow-central-research.tech-441.workers.dev/api/family-portfolio', {
  ...
  assertBookChange(body, previous || JSON.parse(readFileSync(path, 'utf8')));
  return body;
}
