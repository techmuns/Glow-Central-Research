...
test('durable timer arms explicitly, avoids duplicate/inflight jobs and dispatches missed runs',async()=>{
 ...
 const schedule=new BreakoutSchedule(data,{GH_DISPATCH_TOKEN:'fixture',GH_REPO:'techmuns/Glow-Central-Research',GH_REF:'main'}, {now:()=>at,fetcher:async(url,opts={})=>{
...
test('timer restart and GitHub creation delay do not turn 15-minute captures into 30-minute captures',async()=>{
 ...
 const makeSchedule=()=>new BreakoutSchedule(data,{GH_DISPATCH_TOKEN:'fixture',GH_REPO:'techmuns/Glow-Central-Research'}, {now:()=>at,fetcher:async(url,options={})=>{
...
test('in-flight captures recheck in one minute without starting duplicates, including a dispatch race',async()=>{
 ...
 const schedule=new BreakoutSchedule(data,{GH_DISPATCH_TOKEN:'fixture',GH_REPO:'techmuns/Glow-Central-Research'}, {now:()=>at,fetcher:async(url,options={})=>{
...
test('only a signed fixed-workflow main-branch GitHub identity may write',async()=>{
 ...
 const claims={iss:'https://token.actions.githubusercontent.com',aud:BREAKOUT_ENDPOINT,repository:'techmuns/Glow-Central-Research',repository_id:'1339395437',repository_owner_id:'278697674',ref:'refs/heads/main',workflow_ref:'techmuns/Glow-Central-Research/.github/workflows/breakouts-refresh.yml@refs/heads/main',event_name:'schedule',exp:AT/1000+300,iat:AT/1000,nbf:AT/1000,run_id:'1',run_attempt:'1'};
...
test('daily companion files use the exact daily revision and never silently use deployed inputs',async()=>{
 ...
 const response=await handleTechnicals(request,{}, {fetcher:async url=>{assert.equal(url,`https://raw.githubusercontent.com/techmuns/Glow-Central-Research/${sha}/public/data/atr-history.json`);return Response.json(expected);}});
