...
const env = { GH_DISPATCH_TOKEN: 'local-test-secret', GH_REPO: 'techmuns/Glow-Central-Research', GH_REF: 'main' };
let time = Date.parse('2026-09-06T09:00:00Z'), posts = 0, calls = 0, scenario = 'ok', recentAt = null;
const fetcher = async (input, init) => {
  calls++;
  const url = new URL(input);
  assert.equal(url.origin, 'https://api.github.com');
  assert(url.pathname.startsWith('/repos/techmuns/Glow-Central-Research/actions/workflows/telegram-refresh.yml/'));
...
const queueCfg = { token: 'local-test-secret', owner: 'techmuns', repo: 'Glow-Central-Research', ref: 'main', now: () => queueTime };
function queuedAPI({ mode = 'safe', queues = 1, edit = () => {}, jobInventory = { total_count: 0, jobs: [] } } = {}) {
  ...
  const fetcher = async (input, init) => {
    ...
    if (init.method === 'POST') {
      assert.equal(url.pathname, '/repos/techmuns/Glow-Central-Research/actions/workflows/telegram-refresh.yml/dispatches');
