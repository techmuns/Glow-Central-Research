...
/**
 * Institutions — ONE TABLE OF REAL FILED SHAREHOLDINGS.
 *
 * The columns, their order and the sortable headings mirror Trendlyne's own page: Stock, Holding
 * Value, Qty Held, then the latest quarter's change and holding percentage followed by eight
 * prior quarters. Everything it renders lives in js/investors/filed.js.
 *
 * THE SYNTHETIC FUNDS ARE NOT HERE. The seven institutions in institutions.json have real names
 * and invented positions, and they used to fill this view. Putting them under the filed table
 * meant an amber ribbon and a card grid below a real one — two provenances in one sub-view, which
 * is the thing this repo removes rather than labels. They are gone from the page; that only one
 * fund is wired is stated in the note under the table, in the Filed pill's modal and in the
 * Sources modal. `institutions.json` still feeds Fund Flows' category charts.
 */
function renderInstitutions(ctx) {
  ...
  if (!panel.html) {
    // No filed file on disk. Fall back to the synthetic view rather than an empty tab, and let its
