...
import {
  ...
  sourceLinks,
  IPO_REPOSITORY,
} from '../data/ipo-monitor-shared.js';
...
const link = (name, value) => {
  ...
  return url ? `<a href="${e(url)}" target="_blank" rel="noopener noreferrer">${e(name)} ↗</a>` : '';
};
const chip = (value, muted = false) => `<span class="ipo-chip${muted ? ' muted' : ''}">${e(value)}</span>`;
const dataStatus = (row) => {
  const n = ['rev_growth_pct', 'ebitda_margin_pct', 'pat_growth_pct', 'pat_margin_pct'].filter(
    (k) => numeric(row.financials?.[k]?.value) != null,
  ).length;
  return n === 0 ? 'Awaiting financials' : n === 4 ? 'Complete (4 metrics)' : 'Partial';
};
...
export function render(ctx) {
  ...
  const state = {
    mode: ['weekly', 'tracker', 'news', 'documents'].includes(ctx.params?.view) ? ctx.params.view : 'weekly',
    q: ctx.params?.company || '',
    stage: '',
    lifecycle: '',
    board: '',
    sector: '',
  ...
  ctx.root.innerHTML = `<link rel="stylesheet" href="css/ipo-monitor.css"><div class="ipo-board" data-ipo-board>
    ...
    <div class="ipo-modes">${[
      ...
      .map(
        ([key, title]) =>
          `<button data-ipo-mode="${key}" aria-pressed="${state.mode === key}">${title}</button>`,
      )
      .join('')}</div>
  ...
  const root = ctx.root.querySelector('[data-ipo-board]'),
    view = root.querySelector('[data-ipo-view]');
  const say = (value) => {
    root.querySelector('[data-ipo-freshness]').textContent = value;
  };
  function rebuild() {
    rows = buildIpoRows([...feed.state.snapshots.values()], tracked);
  }
  function freshness() {
    const s = feed.state,
      b = s.bundle;
  ...
  function filtered() {
    return rows
      .filter(
        (row) =>
          matchesIpo(row, state.q) &&
          (!state.lifecycle || row.lifecycle === state.lifecycle) &&
          (!state.stage || row.stage === state.stage) &&
          (!state.board || row.board === state.board) &&
          (!state.sector || row.sector === state.sector) &&
          (!state.type || row.filingType === state.type) &&
          (!state.from || row.filingDate >= state.from) &&
          (!state.to || (row.filingDate && row.filingDate <= state.to)),
      )
      .sort((a, b) =>
        state.order === 'name'
          ? a.name.localeCompare(b.name)
          : state.order === 'score'
            ? (scoreIpo(b.financials, config).total ?? -1) - (scoreIpo(a.financials, config).total ?? -1)
  ...
  function rowMarkup(row, weekly = false) {
    ...
    return `<tr data-ipo-row><td><button class="ipo-company" data-ipo-detail="${idx}" aria-expanded="false">${e(row.name)}</button>${row.tracked ? `<p>${chip('Tracked issuer')}</p>` : ''}<p class="ipo-meta">${e(row.symbol || '')}</p></td>
      ...
      <td>${sourceLinks(row)
        .slice(0, 2)
        .map((s) => link(s.label, s.url))
        .join('<br>')}<p><button data-ipo-documents="${idx}">Documents</button></p></td></tr>`;
  }
  function tableMarkup(display, weekly = false) {
    const headings = [
      'Company',
      weekly ? 'Weekly event' : 'Lifecycle',
      'Filing / date',
      'Reported stage',
      'Sector / board',
      ...(!weekly ? ['Issue / timetable'] : []),
      ...(state.metrics ? ['Rev growth', 'EBITDA margin', 'PAT growth', 'PAT margin'] : []),
      'Score',
      'Research priority',
      'Data status',
      'Source / action',
  ...
  function wireTable() {
    view.querySelectorAll('[data-ipo-detail]').forEach((button) =>
      button.addEventListener('click', () => {
        ...
        parent.insertAdjacentHTML(
          'afterend',
          `<tr class="ipo-detail-row"><td colspan="${parent.children.length}">${detail(rows[Number(button.dataset.ipoDetail)])}</td></tr>`,
        );
      }),
    );
    view
      .querySelectorAll('[data-ipo-score]')
      .forEach((button) =>
        button.addEventListener('click', () => showScore(rows[Number(button.dataset.ipoScore)])),
      );
    view.querySelectorAll('[data-ipo-documents]').forEach((button) =>
      button.addEventListener('click', () => {
        state.q = rows[Number(button.dataset.ipoDocuments)].name;
  ...
  function detail(row) {
    const f = row.filing || {},
      ...
      issue = f.issue || {};
    const kv = (object) =>
      `<dl>${Object.entries(object)
        .map(
          ([k, v]) =>
            `<dt>${e(label(k))}</dt><dd>${v == null || v === '' ? '—' : e(typeof v === 'object' ? JSON.stringify(v) : v)}</dd>`,
        )
        .join('')}</dl>`;
    return `<div class="ipo-detail"><h3>${e(row.name)}</h3>${
      row.tracked
        ? `<p class="ipo-note">${e(row.tracked.note)} Checked ${e(row.tracked.checked_at)}. ${Object.entries(
            row.tracked.sources,
          )
            .map(([k, v]) => link(label(k), v))
            .join(' · ')}</p>`
        : ''
    }
      ...
      <h3>Filing history · ${row.history.length} captured records</h3><div class="ipo-table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Source</th></tr></thead><tbody>${row.history
        .map(
          (h) =>
            `<tr><td>${e(h.filing_date || '—')}</td><td>${e(h.filing_type || '—')}</td><td>${
      ...
      <h3>Financials · original fiscal-year labels and provenance</h3><div class="ipo-table-wrap"><table><thead><tr><th>Metric</th><th>Value</th><th>Source</th><th>Confidence</th></tr></thead><tbody>${
        Object.entries(row.financials)
          .map(
            ([k, mv]) =>
              `<tr><td>${e(label(k))}</td><td>${metric(mv?.value)}</td><td>${e(mv?.source || 'Not supplied')}</td><td>${e(mv?.confidence || 'Not supplied')}${mv?.confidence === 'low' || mv?.source === 'WEB' ? ' · Verify' : ''}</td></tr>`,
          )
          .join('') || '<tr><td colspan="4">Financials not supplied. No score inferred.</td></tr>'
      }</tbody></table></div>
  ...
  function tracker() {
    ...
    view.innerHTML = `<h3>Full Tracker · current & historical captures</h3><p class="ipo-meta" data-ipo-history></p>
      ...
      <label>Sort<select data-ipo-filter="order">${[
        ['date', 'Latest activity'],
        ['name', 'Company'],
        ['score', 'Score'],
      ]
        .map(([v, t]) => `<option value="${v}"${state.order === v ? ' selected' : ''}>${t}</option>`)
        .join('')}</select></label>
    ...
    view.querySelector('[data-ipo-search]').addEventListener('input', (event) => {
      state.q = event.target.value;
      trackerRows();
    });
    view.querySelectorAll('[data-ipo-filter]').forEach((input) =>
      input.addEventListener('change', () => {
        state[input.dataset.ipoFilter] = input.value;
        trackerRows();
      }),
    );
    view.querySelector('[data-ipo-metrics]').addEventListener('change', (event) => {
      state.metrics = event.target.checked;
      trackerRows();
    });
    view.querySelector('[data-ipo-reset]').addEventListener('click', () => {
      Object.assign(state, {
        q: '',
        stage: '',
        lifecycle: '',
  ...
  function trackerRows() {
    ...
    for (const key of ['stage', 'board', 'sector', 'type']) {
      ...
      const values = [
        ...new Set(rows.map((r) => r[key === 'type' ? 'filingType' : key]).filter(Boolean)),
      ].sort();
      select.innerHTML =
    ...
    view.querySelector('[data-ipo-history]').textContent =
      `${feed.state.snapshots.size} of ${feed.state.bundle.historyDates.length || '?'} published snapshots loaded. ${feed.state.failedDates.size ? `${feed.state.failedDates.size} failed: retry available. ` : ''}${feed.state.localDates.size ? `${feed.state.localDates.size} use bundled copies. ` : ''}Older observations retain their own as-of dates.`;
    view.querySelector('[data-ipo-count]').textContent =
      `${displayed.length} of ${rows.length} issuers shown · all imported snapshot records remain available in expanded history.`;
    view.querySelector('[data-ipo-facets]').innerHTML = ['', 'Pre-IPO', 'IPO market', 'Listed / inactive']
      .map(
        (s) =>
          `<button data-ipo-life="${s}" aria-pressed="${state.lifecycle === s}">${s || 'All'} · ${rows.filter((r) => matchesIpo(r, state.q) && (!s || r.lifecycle === s)).length}</button>`,
      )
      .join('');
    view.querySelectorAll('[data-ipo-life]').forEach((b) =>
      b.addEventListener('click', () => {
        state.lifecycle = b.dataset.ipoLife;
        trackerRows();
      }),
    );
    view.querySelector('[data-ipo-table]').innerHTML = tableMarkup(displayed);
    wireTable();
  }
  async function history() {
    const button = view.querySelector('[data-ipo-history-load]');
    ...
    if (state.mode === 'tracker') {
      trackerRows();
      const next = view.querySelector('[data-ipo-history-load]');
      if (next) next.disabled = false;
    }
  }
  function weekly() {
    const current = feed.state.bundle.latest,
      stats = weeklyIpoStats(current, config);
    const prev = feed.state.snapshots.get(current.meta.previous_snapshot_id),
      prior = prev ? weeklyIpoStats(prev, config) : null;
    ...
    view.innerHTML = `<h3>Week ending ${e(current.meta.week_end)} · filing monitor</h3><div class="ipo-cards">${[
      ['drhp', 'New DRHPs'],
      ['prospectus', 'New prospectuses'],
      ['updated', 'Updated / corrected'],
      ['dig', 'Dig deeper'],
    ]
      .map(
        ([k, title]) =>
          `<div class="ipo-card"><strong>${stats[k]}</strong><span>${title}</span><br><small>${prior ? `${stats[k] - prior[k] >= 0 ? '+' : ''}${stats[k] - prior[k]} vs prior captured week` : 'Prior comparable count unavailable'}</small></div>`,
      )
      .join('')}</div>
    ...
    view.querySelector('[data-ipo-open-tracker]').addEventListener('click', () => {
      state.mode = 'tracker';
      draw();
      void history();
    });
  ...
  function newsView() {
    ...
    const stories = [
      ...marketNews.rows().map((r) => ({ ...r, feed: 'News' })),
      ...twitterNews.rows().map((r) => ({ ...r, feed: 'X' })),
    ];
    const renderStories = () => {
      ...
      const selected = stories
        .filter(
          (r) =>
            ipoStory(r) && (!query || `${r.title || ''} ${r.summary || ''}`.toLowerCase().includes(query)),
        )
        .sort((a, b) => (b.publishedAt || '').localeCompare(a.publishedAt || ''));
      view.querySelector('[data-ipo-stories]').innerHTML =
        selected
          .map(
            (r) =>
              `<article class="ipo-story">${chip(r.feed)} <span class="ipo-meta">${e(r.publisher || r.displayName || r.handle || '')} · ${e(r.publishedAt || 'Time not supplied')}</span><p>${link(r.title, r.url)}</p></article>`,
          )
          .join('') ||
        '<p class="ipo-empty">No matching items in the loaded captures. This does not mean there is no IPO news or X discussion.</p>';
    };
    ...
    view.querySelector('[data-ipo-news-query]').addEventListener('input', (event) => {
      state.q = event.target.value;
      renderStories();
    });
    renderStories();
  ...
  async function draw() {
    ...
    documentDispose = null;
    root
      .querySelectorAll('[data-ipo-mode]')
      .forEach((b) => b.setAttribute('aria-pressed', String(b.dataset.ipoMode === state.mode)));
    if (state.mode === 'documents') {
      documentDispose = mountDrhpDocuments({ root: view });
      view.querySelector('[data-drhp-company]').value = state.q;
      return;
    }
    if (!feed.state.bundle) {
      view.innerHTML = '<p class="ipo-empty">Reading captured IPO data…</p>';
      return;
    }
    rebuild();
    if (state.mode === 'weekly') weekly();
      ...
      if (!newsReady) {
        view.innerHTML = '<p class="ipo-empty">Reading news and X captures…</p>';
        await Promise.allSettled([marketNews.load(), twitterNews.load()]);
        if (dead || state.mode !== 'news') return;
        newsReady = true;
      }
      newsView();
    }
  ...
  async function load() {
    ...
    try {
      ...
      if (state.mode === 'tracker') await history();
        ...
        await feed.loadHistory(controller.signal, 1);
        if (!dead && mine === generation && state.mode === 'weekly') {
          rebuild();
          weekly();
        }
      }
    } catch {
      if (!dead && mine === generation)
        say(
          'IPO data could not be read, including the bundled capture. Retry; this is not an empty IPO market.',
        );
    } finally {
      if (!dead && mine === generation) root.querySelector('[data-ipo-refresh]').disabled = false;
    }
  }
  function settings() {
    if (!config) return;
    ...
    openModal(
      `<div class="ipo-board"><button data-modal-close aria-label="Close scoring settings">Close</button><h2>Scoring settings</h2><p>Based on your DRHP dashboard’s model v${e(config.version)}. Changes are local previews for this tab session; no repository or shared model is published.</p><form data-ipo-score-settings><div class="ipo-score-form">${Object.entries(
        draft.components,
      )
        .map(([k, p]) =>
          ['weight', 'floor', 'saturation']
            .map(
              (field) =>
                `<label>${e(p.label)} · ${field}<input type="number" step="any" required data-component="${e(k)}" data-field="${field}" value="${p[field]}"></label>`,
            )
            .join(''),
        )
        .join(
          '',
        )}<label>Minimum coverage<input type="number" required data-config="min_coverage_weight" value="${draft.min_coverage_weight}"></label><label>Dig deeper threshold<input type="number" required data-threshold="dig_deeper_min" value="${draft.thresholds.dig_deeper_min}"></label><label>Monitor threshold<input type="number" required data-threshold="monitor_min" value="${draft.thresholds.monitor_min}"></label></div><p data-score-error role="status"></p><div class="ipo-actions"><button type="submit" class="ipo-primary">Apply local preview</button><button type="button" data-score-reset>Restore repository model</button></div></form></div>`,
    );
    const form = document.querySelector('[data-ipo-score-settings]');
    form.addEventListener('submit', (event) => {
      event.preventDefault();
      form.querySelectorAll('[data-component]').forEach((input) => {
        draft.components[input.dataset.component][input.dataset.field] =
          input.value === '' ? null : Number(input.value);
      });
      form.querySelectorAll('[data-config]').forEach((input) => {
        draft[input.dataset.config] = Number(input.value);
      });
      form.querySelectorAll('[data-threshold]').forEach((input) => {
        draft.thresholds[input.dataset.threshold] = Number(input.value);
      });
      try {
        ...
        form.querySelector('[data-score-error]').textContent =
          'Weights must total 100; bounds, thresholds and coverage must be valid.';
      }
    });
    form.querySelector('[data-score-reset]').addEventListener('click', () => {
      config = feed.state.bundle.config;
      customConfig = false;
      closeModal();
      freshness();
      void draw();
  ...
  function exportCsv(display) {
    ...
    const lines = [
      ...
      ...display.map((r) => [
        ...
        r.board,
        r.sector,
        r.market?.issue_size_cr ?? r.filing?.issue?.total_cr,
        scoreIpo(r.financials, config).total,
        scoreIpo(r.financials, config).bucket,
        dataStatus(r),
        r.history.length,
        sourceLinks(r)
          .map((s) => s.url)
    ...
    a.href = url;
    a.download = `sattva-ipo-tracker-${feed.state.bundle.latest.meta.data_as_of}.csv`;
    a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  root.querySelectorAll('[data-ipo-mode]').forEach((b) =>
    b.addEventListener('click', () => {
      state.mode = b.dataset.ipoMode;
      void draw();
      if (state.mode === 'tracker' && feed.state.bundle) void history();
    }),
  );
  ...
  root.querySelector('[data-ipo-refresh]').disabled = true;
  const trackedPromise = fetch('data/ipo-tracked-issuers.json', {
    signal: AbortSignal.any([controller.signal, AbortSignal.timeout(8000)]),
  })
    .then((r) => {
      if (!r.ok) throw new Error('Supplement unavailable');
      return r.json();
    })
    .then((d) => {
      if (!Array.isArray(d?.issuers)) throw new Error('Invalid supplement');
      if (!dead) tracked = d.issuers;
    })
    .catch(() => {
