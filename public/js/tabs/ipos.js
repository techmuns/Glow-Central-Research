...
export function render(ctx) {
  ...
  const filtered = () => {
    ...
    return feed.rows().filter((r) => r.filingDate && r.filingDate >= from && r.filingDate <= today);
  };
  function paint() {
    if (dead) return;
    ...
    const status = busy ? 'Checking official sources…' : m.liveFailed ? 'Live sources unavailable · retained captures shown' : m.degraded ? 'Partial / dated coverage' : 'Official sources checked';
    ...
    const table = scoreTable({
      ...
      columns: [
        { label: 'Filed (IST)', get: (r) => when(r.filingDate), sortValue: (r) => r.filingDate || '', align: 'left' },
      ...
      initialSort: { key: 'Filed (IST)', dir: 'desc' }, initialView: view,
    ...
    ctx.root.innerHTML = `<section data-ipo-filings>
      ${sectionHead({ title: 'IPO Filings', description: 'DRHPs, red herring prospectuses, final prospectuses and related public-issue documents. All issuers, including unlisted companies; Portfolio / Watchlist scope does not filter this tab.', meta: pill({ label: `${m.count.toLocaleString('en-IN')} captured filings`, tone: 'brand' }) })}
