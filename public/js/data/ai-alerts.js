...
// Two exchange-filing feeds are ONE independent source, and so are the two news routes: a company
// filing the same thing to NSE and BSE is not two opinions. Exported so a card can tag each evidence
// row with the family it counts as, rather than with the label it prints — the two legitimately
// differ, and a check comparing printed labels against counted families reads that as a card
// claiming fewer sources than it shows.
export const feedFamily = (event) => event.feed === 'nse-filings' ? 'announcements' : event.feed === 'market-news' ? 'news' : event.feed;
...
function* rankSteps(report, { holdings = coverage.holdings(), positionSizes = null, insightCompanies = screenerInsights.all(), companyMetadata = technicals.all().map(row => row.company) } = {}) {
  ...
  const input = { day, scope: report?.scope || 'universe', events,
    ...
    insights: insightCompanies, sectors: companyMetadata, session: JSON.stringify([token, email, orgId]) };
  const cached = rankCache.find(entry => entry.input.day === day && entry.input.scope === input.scope &&
    ...
    entry.input.session === input.session && sameRows(entry.input.events, events) &&
  ...
  rankCache.unshift({ input: { ...input, events: [...events], insights: [...insightCompanies], sectors: [...companyMetadata] }, result });
