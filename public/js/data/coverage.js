...
export function syncLabel() {
  ...
  if (raw?.sourceKind === 'glow-statements') return `GlowVentures statement book · statements as of ${family?.asOf || raw.asOf} · source ${raw.sourceCommit?.sha || 'published snapshot'} · ${family?.checking ? 'checking now' : `checked ${family?.checkedAt || raw.syncedAt || 'not yet'}`}. Values and ownership are statement-based; later trades require a new statement. ${syncError || ''}`;
  if (family) return family.checking ? 'Checking Family Office for changes — showing the last verified holdings.' : 'Using the revalidated GlowVentures statement book. Statement dates remain the portfolio dates; this is not a live broker feed.';
...
export function meta() {
  ...
  return {
    ...
    source: family ? 'GlowVentures statement book' : raw?.source || null,
