...
const historyIndex = (dates) => {
  if (!Array.isArray(dates) || dates.some((date) => !validDay(date))) throw new Error('Invalid history index');
  return [...new Set(dates)].sort().reverse();
};

export function createIpoFeed({ fetcher = fetch } = {}) {
  const state = { bundle: null, snapshots: new Map(), failedDates: new Set(), localDates: new Set(), fallback: false, error: null };
  const read = async (url, signal) => {
    const deadline = AbortSignal.any([...(signal ? [signal] : []), AbortSignal.timeout(url.startsWith('api/') ? 22000 : 8000)]);
  ...
  async function load(signal) {
    ...
    try {
      ...
      state.fallback = false; state.error = null;
    } catch (error) {
      if (signal?.aborted) throw error;
      const [latest, config, index] = await Promise.all(['latest.json', 'scoring_config.json', 'index.json'].map((path) => read(`data/ipo-monitor/${path}`, signal)));
      bundle = { latest: validateIpoSnapshot(latest), config: validateScoring(config), historyDates: historyIndex(index.historyDates), historyAvailable: true, checkedAt: null, source: `Bundled DRHP snapshot · ${index.sourceCommit}` };
      state.fallback = true; state.error = 'The current published source could not be read. Showing the bundled capture.';
    }
  ...
  async function loadHistory(signal, limit = 20) {
    const dates = (state.bundle?.historyDates || []).filter((date) => !state.snapshots.has(date) || state.failedDates.has(date)).slice(0, limit);
    // Small bounded pool; loading the tracker never starts the upstream capture pipeline.
    let next = 0;
    await Promise.all(Array.from({ length: Math.min(3, dates.length) }, async () => {
      while (next < dates.length && !signal?.aborted) {
        ...
        try {
          ...
          state.snapshots.set(date, snapshot); state.failedDates.delete(date);
        } catch { if (!signal?.aborted) state.failedDates.add(date); }
      }
    }));
    return state;
  }
