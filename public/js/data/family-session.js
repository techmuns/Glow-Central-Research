...
export function refreshFamilySession() {
  if (paused || portfolioReadBusy() && !refreshing) return Promise.resolve(null);
  if (!refreshing) refreshing = readPositionSizes().catch(() => null).finally(() => { refreshing = null; });
  return refreshing;
}
export function startFamilySession() {
  ...
  started = true;
  const refresh = () => {
    if (!document.hidden && navigator.onLine !== false) void refreshFamilySession();
  };
  onPortfolioConnection(connected => { if (connected) refresh(); });
  onPortfolioReady(refresh);
  live.register('family-session', { intervalMs: 60000, fetcher: async () => {
    if (paused || portfolioReadBusy()) return { checked: 0 };
    const reply = await refreshFamilySession();
