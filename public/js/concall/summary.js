...
<<<<<<< HEAD
    return `<div class="table-scroll-surface overflow-x-auto"><table data-column-layout="summary:1" class="w-full text-left text-sm text-slate-700"><tbody>${block.rows.map(row => `<tr class="border-b border-slate-100">${row.map(cell => `<td class="px-3 py-2 align-top">${e(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
=======
