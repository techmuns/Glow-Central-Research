...
import { IPO_SOURCE_IDS, IPO_POLL_MS, filingUrl } from '../data/ipo-filings-shared.js';
...
const stamp = (at) => Number.isFinite(Date.parse(at))
  ...
  : 'Not yet checked';
const cadence = 'Checks on opening or returning to IPOs, then every five minutes while visible. Shared responses may be cached for five minutes. No continuous collection while closed; source publication can lag.';
...
export function ipoSourceGroup(meta = feed.meta(), now = Date.now()) {
  ...
  return {
    ...
    items: IPO_SOURCE_IDS.map((id, index) => {
      ...
      const stale = !Number.isFinite(at) || now - at > IPO_POLL_MS * 2 || at > now + 60000;
      ...
      const readLabel = { unchecked: 'Not checked', unavailable: 'Unavailable', unconfirmed: 'Unconfirmed', dated: 'Dated', partial: 'Partial', read: 'Read' }[readState];
      const details = [
