...
export function wireIntegrity(root, disposers, openInvestor, openManager) {
  ...
  function paint() {
    ...
    host.innerHTML = `<details class="mb-4 rounded-xl bg-amber-50 p-3 text-xs text-amber-900 ring-1 ring-amber-200" ${expanded ? 'open' : ''}>
      ...
      ${publicReport ? `<p class="mt-2">${publicReport.coverage.parsed.toLocaleString('en-IN')} exchange filings read across ${publicReport.coverage.companies.toLocaleString('en-IN')} companies · ${publicReport.holdings.length.toLocaleString('en-IN')} attributed disclosures · checked ${esc(shownDate(publicReport.captureCheckedAt))}.</p>` : ''}
